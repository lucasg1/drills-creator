...
def create_visualization_for_hand(args):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Couldn't load original file: {original_file}. Creating minimal structure: {e}")
        ...
        logger.error(f"Error creating visualization for {hand_json_path}: {e}", exc_info=True)
...
class HandImageGenerator:
    ...
    def run(self):
        ...
        if not hand_json_files:
            logger.warning("No hand JSON files found. Make sure the input directory is correct.")
            return
            
        # If we're processing a specific hand, just print the files we found
        if self.specific_hand and len(hand_json_files) > 0:
            logger.info(f"Found {len(hand_json_files)} files for hand {self.specific_hand}:")
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_path)):
                try:
                    ...
                    logger.error(f"Error processing {hand_json_path}: {e}", exc_info=True)
                    self.stats["error_files"] += 1
                
                # Log progress every 50 files
                if (i + 1) % 50 == 0:
                    logger.info(f"Progress: {i+1}/{len(hand_json_files)} files processed")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate images from hand JSON files"
    )
    ...
    parser.add_argument(
        "--output",
        default="hand_images",
        help="Directory to save hand images"
    )
    parser.add_argument(
        "--game-type",
    ...
    parser.add_argument(
        "--hand",
        help="Generate image for a specific hand only (e.g., 'AKs', 'TT')"
    )
    parser.add_argument(
        "--file",
        help="Generate image for a specific hand JSON file (absolute path)"
    )
    ...
    if args.file:
        ...
        hand_to_cards_map = {}
        result = create_visualization_for_hand((file_path, output_dir, hand_to_cards_map))
        logger.info(result)
        return
